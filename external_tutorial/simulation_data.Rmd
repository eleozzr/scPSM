...
<!-- rmarkdown::render("~/Downloads/simulation_data.Rmd", rmarkdown::md_document(variant = "markdown_github")) -->
...
Set parameters: 1000 genes, one cell type, two batches, 100 cells for each batch, almost no batch effect, 
and additional dropout to the whole dataset.
...
Set parameters: 1000 genes, two cell types, two batches, batch.factor=0.8, de.probability=0.3.
```{r}
params <- newSplatParams()
params <- setParam(params, "nGenes", 1000)
...
Set parameters: batch.factor = 0.01, 0.05, 0.1, 0.5. Create counts in Seurat object.
```{r}
