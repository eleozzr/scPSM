...
{
  prep.out <- .prepare_input_data(batches, cos.norm.in=T, cos.norm.out=T, 
  ...
  for (mdx in seq_len(nmerges)) {
    ...
    psm.datas <- lapply(1:n_right, function(x){
      as.data.frame(psm.data[c(parings[[x]]$left, parings[[x]]$right+n_left),])
    })
    ...
    right.extras.cor <- sapply(1:n_right, function(x){
      if (length(parings[[x]]$left) > 1) {
        colSums(w.logit[[x]]*left.extras[parings[[x]]$left, ])/sum(w.logit[[x]])
      } else {
        right.extras[x,]
      }
    }) 
    right.data.cor <- sapply(1:n_right, function(x){
      if (length(parings[[x]]$left) > 1) {
        colSums(w.logit[[x]]*left.data[parings[[x]]$left, ])/sum(w.logit[[x]])
      } else {
        right.data[x,]
      }
    })
